...
extern "C"
{
			 ...
			 void ** ptrUserData, int nThreadIndex, void * pAppPtr)
	{
		...
		static bool   q[7] = {0,0,0,0,0,0,0}; // Switch Array (Dimension = N?Switch)
		...
		static double F1,F2,F3,F4,F5,F6,xa1,xb1,ei1a,xaa1,ei1;
		...
		///////////////////////////////////
		// Input
		egs 	     = in[1];	// Grid Voltage
		igs          = in[2];	// Grid Current
		vCs		  	 = in[3];   // 3L Bus Voltage
		vtris  		 = in[0];	// Triangular
...
//			vl_ref = 110.*sqrt(2)*sin(theta); // Open Loop
		...
		if(vse0_ref   >= vtris) ql = 1.0; else ql = 0.0;
		/*
